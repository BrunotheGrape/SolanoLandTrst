...
'''
This beta version of the code creates all of the requests upfront and retrieves after the imagery is ready for download.
...
def main(argv):
	...
	parser.add_argument('-product_name',help='product_name NC,CIR,NDVI,ZONE,TIRS,MULTIBAND FULL',nargs='?',default=None)
	parser.add_argument('-color_map',help='color_map N-AVHRR N-R3 N-R2 N-GREENG',nargs='?',default='N-AVHRR')
	parser.add_argument('-task_list',help='task_list',nargs='?',default=None)
	parser.add_argument('-RGB_color_map',help='RGB_color_map',nargs='?',default=None)
	parser.add_argument('-output_csv',help='output_csv filename',nargs='?',default=None)
	parser.add_argument('-error_only_logging',help='error_only_logging',nargs='?',default=True)
	parser.add_argument('-rolling_window',help='rolling_window',nargs='?',default=None)
	parser.add_argument('-rollback_start_days',help='rollback_start_days',nargs='?',default=1)
	parser.add_argument('-sub_folder_by_block_product',help='sub_folder_by_block_product',nargs='?',default=None)
	...
	if task_list and access_token:
		...
		for product_name in product_name_list:
			run_download_workflow(downloading_report,working_dir,year_week,product_name,block_dic_struct,username,access_token,color_map,RGB_color_map,error_only_logging)
...
def download_imagery_rw_deamon(working_dir,username,access_token,rollback_start_days,color_map,sub_folder_by_block_product=None):
	...
	while True:
		...
		master_layer_list=[]
		for product_name in product_name_list:
			layer_list=get_layer_by_add_date(block_dic_struct,product_name,username,access_token,addEpochStart,addEpochEnd)
		...
		if master_layer_list:
			...
			for layer_info in master_layer_list:
				if sub_folder_by_block_product:
					clean_block_name=clean_filename(layer_info['block_name']+'_'+layer_info['product'])
					working_sub_dir=os.path.join(working_dir,clean_block_name)
					...
					try:
						if 'status' not in request_info:
							...
							try:
								...
								download_file(request_info['download_url'],download_filename)
...
def get_all_layers_list(block_dic_struct,product_name,username,access_token):
	...
	return layer_list
def get_layer_by_add_date(block_dic_struct,product_name,username,access_token,addEpochStart,addEpochEnd):
...
class DownloadingReport(object):
	def __init__(self,csv_filename=None):
		...
		self.csv_header_list=['output_filename','status','request_url','download_url','timestamp','message']
